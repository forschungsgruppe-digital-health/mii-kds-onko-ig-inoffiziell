// Template aliases the MODULE does not define itself.
// The module's own input/fsh/aliases.fsh wins wherever both define a name; the 1
// alias(es) it also defines are omitted here ($observation-category) because a duplicate Alias is a
// SUSHI error. The rest are kept because template rulesets that stay in this module
// (crmi, license, extension-context, test-data-label) reference them.

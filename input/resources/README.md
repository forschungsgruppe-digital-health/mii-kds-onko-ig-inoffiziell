...
1. **Replace `onko` inside the file** — it appears in the resource
   `id` (`mii-param-onko-manifest`). A `{{…}}` left in a FHIR `id` is
   ...
   `input/translations/de/ImplementationGuide-mii-ig-onko.po`, which

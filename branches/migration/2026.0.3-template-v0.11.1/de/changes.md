...
Diese Seite hält die Änderungen zwischen den veröffentlichten Versionen des Moduls **Onkologie** fest, die neueste Version zuerst. Sie folgt [Keep a Changelog](https://keepachangelog.com/de/1.1.0/) und dem KDS-CalVer-Schema, das die Seite [Versionierung](version-history.md) beschreibt.
...
> **Written during migration - review before release.** Die folgenden Release Notes sind ihrem Ursprung nach deutsch und stehen hier unverändert auf Deutsch — auf der englischen wie auf der deutschen Seite —, damit der Änderungsnachweis einer veröffentlichten Version in beiden Sprachen identisch gelesen wird.
...
* `feat` **Frühere Tumorerkrankungen Profil**: Neues Profil **Frühere Tumorerkrankung** (Condition) zur Erfassung anamnestischer Tumorerkrankungen mit Mapping zu oBDS 5.9 
...
* **Erweiterte CarePlan** zur Unterstützung sowohl bisheriger oBDS-Darstellung der Tumorkonferenzen als auch komplexere Darstellung von Therapieempfehlungen wie im Modul Molekulares Tumorboard 
* **Grund**: FHIR R4 Invariant cpl-3 verhindert gleichzeitige Nutzung von `activity.detail.code` und `activity.reference`
* **Lösung**: Activity-Slicing mit `obds` (Standard oBDS 19.1 Kategorisierung) und `extended` (auf **Therapieempfehlung Kombinationstherapie** (RequestGroup) basierende Protokolle) Slices
...
* **Dual-Coding-Support**: **Systemische Therapie** (MedicationStatement) erweitert um UNII-Slice zusätzlich zum bestehenden ATC-Slice 
...
* **Neue Extension**: **Urgency Operation** Extension zur Erfassung der Eingriffsmodalität 
...
* **Component-Kardinalität**: Änderung von `component 1..*` zu `component 0..*` im **Verlauf** (Observation) 
...
* **Präoperative Markierung**: **Mamma Präoperative Markierung** für verschiedene Markierungsmodalitäten (Draht, Seeds, magnetisch)
...
* **MRT Mesorektale Faszie**: **KRK MRT mesorektale Faszie** mit Abstandsmessung (oBDS KR2)
...
* **Malignes Melanom**: Vollständige Implementierung des **Malignes-Melanom-Moduls** 

...
| Active as of 2026-08-22 | *Computable Name*:MII_PR_Onko_TNM_a_Symbol |
...
The a symbol can be prepended to every TNM. It signals that the observation was made on the occasion of an autopsy.

> **Written during migration - review before release.** Condensed from the source page's search-parameter list (one numbered entry with an example query per parameter): for this profile the search parameters `_id`, `_profile`, `status`, `code`, `subject`, `focus`, `encounter`, `date` and `derived-from` MUST be supported, also in combination. The source page also rendered mappings to the oncology logical model and to the [Einheitlicher onkologischer Basisdatensatz (oBDS)](https://basisdatensatz.de/basisdatensatz); those mappings are part of the profiles themselves.

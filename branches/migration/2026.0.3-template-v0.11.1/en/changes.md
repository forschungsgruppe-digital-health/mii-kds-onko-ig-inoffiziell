...
This page records the changes between the released versions of the **Onkologie** module, newest version first. It follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) and the KDS calendar versioning scheme described on the [Versioning](version-history.md) page.
...
> **Written during migration - review before release.** The release notes below are German by origin and are reproduced here in German, unchanged, on both the English and the German page, so that the record of a released version reads identically in either language.
